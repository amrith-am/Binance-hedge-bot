...
# Binance Web Socket
SOCKET = 'wss://fstream.binance.com/ws/bnbusdt@kline_1m'
# Parameters of the RSI Strategy
RSI_PERIOD = 2
RSI_OVERBOUGHT = 70
RSI_OVERSOLD = 30
# Symbol of Ethereum
TRADE_SYMBOL = "BNXUSDT"
...
def on_message(ws, message):
    ...
    if closed_candle:
        ...
        df = df.append({'date': datetime.fromtimestamp(
            candle['t']/1000), 'close price': float(candle['c'])}, ignore_index=True)
    ...
    if len(df) > RSI_PERIOD:
        ...
        if last_rsi > RSI_OVERBOUGHT:
            ...
            if in_position:
                ...
                # Binance Sell Order
                # order_success = order(
                # TRADE_SYMBOL, TRADE_QUANTITY, SIDE_SELL, ORDER_TYPE_MARKET)
        ...
        #Symbol is Oversold
            ...
            #If not in position
                ...
                # Binance Buy Order
                # order_success = order(
                # TRADE_SYMBOL, TRADE_QUANTITY, SIDE_BUY, ORDER_TYPE_MARKET)
                # In position
                in_position = True
    return in_position
